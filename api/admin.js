...
api.post('/join',
		function (req, res) {
			...
			db.query(sql, data, function (err, result) {
				...
				res.status(200).json({status:200, data: {UID: adminUID, token: token}, message:"success"});
			});
		}
);
...
api.put('/password',
		...
		function (req, res) {
			...
			const exec = db.query(sql, adminUID, function (err, result) {
				if (err) throw err;
				...
				if(password != adminPasswd)
					res.status(403).json({status:403, data:"false", message:"기존 비밀번호가 틀렸습니다."});
				else if(newPasswd == adminPasswd)
					res.status(403).json({status:403, data:"false", message:"기존 비밀번호와 동일합니다."});
				else{
					...
					const exec2 = db.query(update_sql, data, function (err, result) {
						...
						res.status(200).json({status:200, data:"true", message:"변경되었습니다."});
					});

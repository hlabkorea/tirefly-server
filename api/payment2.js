...
api.post('/monthPayment',
    ...
    async function (req , res, next) {
        try {
            ...
            const nowMonthProduct = await selectMonthPayment('product', 0);
            const lastMonthProduct = await selectMonthPayment('product', 1)
            const last2MonthProduct = await selectMonthPayment('product', 2)
            const nowMonthMembership = await selectMonthPayment('membership', 0)
            const lastMonthMembership = await selectMonthPayment('membership', 1)
            const last2MonthMembership = await selectMonthPayment('membership', 2)


            res.status(200).json({
                status : 200,
                data : {
                    "nowMonthProduct" : nowMonthProduct,
                    "lastMonthProduct" : lastMonthProduct,
                    "last2MonthProduct" : last2MonthProduct,
                    "nowMonthMembership" : nowMonthMembership,
                    "lastMonthMembership" : lastMonthMembership,
                    "last2MonthMembership" : last2MonthMembership,
                },
                message : "success",
            })
        } catch (err) {
